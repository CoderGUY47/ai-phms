...
export function getMockExtraction(fileName?: string): MockDataResult {
  const cleanName = fileName ? fileName.toLowerCase().replace(/[\s\-_()]/g, "") : "";
  
  // load mock prescriptions data dynamically from json file
  const jsonPath = path.join(process.cwd(), "public", "data", "mock-prescriptions.json");
  const raw = fs.readFileSync(jsonPath, "utf-8");
  const mockData = JSON.parse(raw);

  const staticScenarios = mockData.staticScenarios as Record<string, any>;

  // 1. check for predefined files (1, 2, or 3)
  if (cleanName.includes("1") || cleanName.includes("one")) return staticScenarios["1"] as MockDataResult;
  if (cleanName.includes("2") || cleanName.includes("two")) return staticScenarios["2"] as MockDataResult;
  if (cleanName.includes("3") || cleanName.includes("three")) return staticScenarios["3"] as MockDataResult;
